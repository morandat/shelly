...
public class ModelFactory {
	...
	public CommandGroup createModel(Class<?> clazz) {
		fr.labri.shelly.annotations.CommandGroup annotation = clazz.getAnnotation(CMDGRP_CLASS);
		if (annotation == null)
	...
	private class Builder extends TraversalVisitor {
		...
		public CommandGroup build(String name, Class<?> clazz) {
			CommandGroup grp = createCommandGroup(null, name, clazz);
			visit(grp);
			return grp;
		}
		...
		public CommandGroup createCommandGroup(OptionGroup parent, String name, Class<?> clazz) {
			if ((parent == null) != (clazz.getEnclosingClass() == null))
				throw new RuntimeException("Cannot create option group when not starting at top level"); // FIXME
			CommandGroup grp = new CommandGroup(parent, name, clazz);
			return grp;
		}
